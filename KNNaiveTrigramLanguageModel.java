 ...
 */
public class KNNaiveTrigramLanguageModel implements NgramLanguageModel {
    ...
    // Counters
    TIntOpenHashMap<Integer> unigramMap = new TIntOpenHashMap<>(500000);
    TIntOpenHashMap<Long> bigramMap = new TIntOpenHashMap<>(4000000);
    TIntOpenHashMap<Long> trigramMap = new TIntOpenHashMap<>(10000000);

    // Counters for KN Smoothing 
    TIntOpenHashMap<Integer> bigramStartsWithCount = new TIntOpenHashMap<>(500000);
    TIntOpenHashMap<Integer> bigramEndsWithCount = new TIntOpenHashMap<>(500000);
//    TIntOpenHashMap<Integer> trigramStartsWithCount = new TIntOpenHashMap<>(100000);
    TIntOpenHashMap<Long> trigramStartsWithCount = new TIntOpenHashMap<>(500000); // trigram starts with w3w2
    TIntOpenHashMap<Integer> trigramEndsWithCount = new TIntOpenHashMap<>(500000);
    /**
     * Constructor
     ...
     */
    public KNNaiveTrigramLanguageModel(Iterable<List<String>> sentenceCollection) {
...
//        trigramStartsWithCount.increment(indexTrigramWord, 1);
    ...
    public double knSmoothing(int[] ngram, int from, int to){
        ...
        // Base case: Unigram
        if (order == 1) return baseCaseKN(ngram, from);

        int[] context = retrieveContext(ngram);
...
//        System.out.println("KN sum = " + knScore + " and score = " + Math.log(knScore)); // DEBUG
    ...
    private double baseCaseKN(int[] ngram, int from) {
...
//        System.out.println("Base case, count for worddx=" + ngram[from] + " is " + countUnigram + " over " + totalUnigram); // DEBUG
    ...
    private int contextCountKN(int[] ngram, int from, int order) {
        ...
        if (order ==2) {
            ...
            // order == 3, encode
            long trigramEncoded = Assignment1Utility.bitPackingTrigram(ngram[from], ngram[from+1], ngram[from+2]);
            contextCount = trigramMap.get(trigramEncoded);
        }
        return contextCount;
    }
